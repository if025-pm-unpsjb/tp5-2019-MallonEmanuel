...
QueueHandle_t xQueue1to3,xQueue2to3;
...
int main(){
    ...
    xQueue1to3 = xQueueCreate( 10, sizeof(int)); // @suppress("Function cannot be resolved")
...
void thread1 (void* params){
    ...
    while(1){
		...
		if( xQueue1to3 != 0){
				...
				xQueueSend( xQueue1to3, &id, ( TickType_t ) 0 ); // @suppress("Invalid arguments")
				eat_cpu(xExecTime);
...
void thread2 (void* params){
    ...
    while(1){
    	...
    	if( xQueue2to3 != 0 ){
			...
			int after = xTaskGetTickCount();
			if(xSemaphoreTake(xSemaphore, 5000) == pdTRUE){ // @suppress("Invalid arguments")
				pc.printf("Tarea %d [%d , %d, %d] Enviado a=%d\n\r"
						,taskParams->id
						,before
						,after
						,instance
...
void thread3 (void* params){
    ...
    while(1){
    	...
    	msg2 = 0;
    	int before = xTaskGetTickCount();
		if(xQueueReceive( xQueue1to3, &msg, ( TickType_t ) xDeadTime ) == pdTRUE){ // @suppress("Invalid arguments")
			if(xQueueReceive( xQueue2to3, &msg2, ( TickType_t ) xDeadTime )  == pdTRUE){ // @suppress("Invalid arguments")
				eat_cpu(xExecTime);
				int after = xTaskGetTickCount();
				if(xSemaphoreTake(xSemaphore, 5000) == pdTRUE){ // @suppress("Invalid arguments")
